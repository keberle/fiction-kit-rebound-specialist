# Draft Tasks (Actionable Work Items)

**Draft:** 002-episode-02-draft  
**Generated:** 2025-01-13  
**Updated:** 2025-12-25 (REORGANIZED for alternating timelines)  
**Status:** Ready for implementation

---

## REORGANIZATION NOTE (2025-12-25)

**Episode 2 outline and tasks have been REORGANIZED to alternate between timelines for better pacing and passage of time.**

**Key Changes:**
- Old Scene 8 (Bookstore to Speakeasy) → **New Scene 9**
- Old Scene 9 (Prohibition Speakeasy) → **New Scene 11**
- Old Scene 10 (Finish Line) → **New Scene 13**
- Old Scene 11 (Eddie's House) → **New Scene 8**
- Old Scene 12 (Clinic Montage) → **New Scene 10**
- Old Scene 13 (Clinic Success) → **New Scene 12**
- Old Scene 14 (Eddie and Baxter) → **New Scene 14** (unchanged)

**New Alternating Structure:**
- Scenes 1-4: Setup (unchanged)
- Scene 5: Hunt begins → Scene 6: Investigation begins
- Scene 7: Hunt continues → Scene 8: Investigation planning
- Scene 9: Hunt accelerates → Scene 10: Investigation attempts
- Scene 11: Hunt turning point → Scene 12: Investigation success
- Scene 13: Hunt victory → Scene 14: Investigation moral choice
- Scenes 15-18: Convergence (unchanged)

This creates better pacing, stronger sense of time passage, and more dynamic storytelling throughout Act 2.

---

## Pre-Implementation Verification ✅

All prerequisites completed:
- ✅ idea.md updated to 18 scenes
- ✅ clarify.md complete (all questions answered)
- ✅ plan.md regenerated with ensemble structure analysis
- ✅ elements/outlines/episode-02.md finalized at 18 scenes
- ✅ Stephen King craft review completed
- ✅ All character files verified
- ✅ Continuity with Episode 1 confirmed

---

## Task List

### Phase 1: Directory Setup

**T001 — Create episode directory**
- **Action:** Create `content/episodes/episode-02-the-wedding-weekend/` directory
- **Command:** `New-Item -ItemType Directory -Path "content/episodes/episode-02-the-wedding-weekend"`
- **Acceptance:** Directory exists and is empty

---

### Phase 2: Scene Drafting (Sequential 1-18, REORGANIZED with alternating timelines)

**CRITICAL:** Draft scenes in order. Each task represents one scene file. **NEW STRUCTURE alternates between hunt and investigation for better pacing.**

---

#### **T002 — Draft Scene 1: Holding Cell Graffiti**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/01-holding-cell-graffiti.md`
- **Action:**
  - Open with Martinez or Finch mid-question (McDonald-style dialogue-first)
  - Cold metal bench, industrial cleaner smell, bone-tired exhaustion
  - Doogan notices absurd wall graffiti ("Julio ♥ Dominique")
  - Martinez leads (dry, deadpan), Finch spikes (uglier insinuations)
  - Doogan stays on grocery-store story
  - Knock → sidebar → Martinez: Doogan "lawyered up"
  - Button: Finch: "The guilty ones always have lawyers." Hard cut.
- **Beats Source:** elements/outlines/episode-02.md Scene 1
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Dialogue-first opening (no atmospheric intro)
  - Doogan POV maintained (no head-hopping)
  - Martinez established (dry, deadpan, principled)
  - Sensory details present (cold, smell, exhaustion)
  - Opens door: murder suspect status / Closes door: interrogation ends

---

#### **T003 — Draft Scene 2: The Walkout**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/02-the-walkout.md`
- **Action:**
  - Open with Eddie or Hank speaking as Doogan walks out
  - Eddie is there, Hank is there
  - Hank's rules: don't leave town, don't talk, don't get cute
  - Hank mentions money too many times
  - Button: Eddie: "Abby's making pancakes."
- **Beats Source:** elements/outlines/episode-02.md Scene 2
- **Acceptance:**
  - File exists with ~800-1,000 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Hank characterized (parking meter with law degree)
  - Opens door: legal constraints, lawyer costs / Closes door: interrogation phase complete

---

#### **T004 — Draft Scene 3: Eddie's Kitchen**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/03-eddies-kitchen.md`
- **Action:**
  - Open with Eddie mid-sentence motor-mouthing
  - Abby needles Doogan about arrest
  - Eddie steamrolls: Hank costs money, need new job
  - Team splits: Doogan on wedding job, Eddie/Abby/Baxter on Annabelle investigation
  - New client: Thomas and Sylvia Billings, keep Cheryl from detonating wedding
  - Button: Abby: "And showered wouldn't hurt."
- **Character Voice:**
  - **EDDIE CRITICAL:** Motor-mouth avalanche style, mid-sentence starts, em-dashes, tangents circle back
  - **Example:** "—so Hank costs money, obviously, guy's a lawyer even if he's a bottom-tier one, and lawyers don't work for free or friendship or any of the things normal humans accept, which means we need money, which means you need to work, which I know sounds insane considering you were literally arrested last night but guess what, the Billings still want their wedding weekend handled—"
  - **Abby:** Elevated, sharp, pragmatic
- **Sensory:** Doogan barely tastes pancakes (processing trauma)
- **Beats Source:** elements/outlines/episode-02.md Scene 3
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Eddie mid-sentence opening
  - Eddie motor-mouth voice present (NOT short/punchy)
  - Abby voice correct (no yoga pants, elevated style)
  - Doogan using work as distraction shown through physical tells
  - Opens door: new job, team split / Closes door: trauma processing delayed

---

#### **T005 — Draft Scene 4: Restaurant Meeting**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/04-restaurant-meeting.md`
- **Action:**
  - Open with introductions already in progress
  - Thomas corrects "Tom" → "Thomas"
  - Billings describe Cheryl as "handful," want her "contained"
  - They don't fully explain real emotional landmine
  - Doogan fills in assumption, they don't contradict
  - Itinerary: Saturday morning scavenger hunt at country club
  - Sylvia: hunt should be "so much fun" (she designed it)
  - Terry will meet Doogan there
  - Button: Plan set
- **Character Voice:** Eddie motor-mouth throughout
- **Beats Source:** elements/outlines/episode-02.md Scene 4
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Thomas characterized (wealth/control through "Thomas not Tom")
  - Eddie motor-mouth voice present
  - Opens door: job parameters, hidden truth / Closes door: Doogan accepts without full picture

---

#### **T006 — Draft Scene 5: Country Club Kickoff**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/05-country-club-kickoff.md`
- **Action:**
  - Open with Terry mid-introduction or Cheryl speaking
  - Terry introduces himself (flamboyant, doesn't know real job)
  - Terry introduces Cheryl (competitive like religion)
  - Rules: designer handbag prize, embarrassment for second place
  - Hunt is literature-coded (Sylvia's master's degree)
  - First clue: "My mistress' eyes are nothing like the sun;" (Sonnet 130)
  - Cheryl has no idea, Baxter solves (roses on two lines, The Rose Shop nearby)
  - Turn: Doogan taps ear: "On second thought, I'm going to need help."
  - Button: They jog out, Jillian + boyfriend clock them and follow
- **Cheryl Throughline (Scene 1 of 5):**
  - Sharp edge to competitiveness (proving something, not just fun)
  - Hint at sister/wedding tension through tone
  - Doogan notices but doesn't press
- **Emotional State:** Doogan running on fumes, exhaustion showing through physical tells
- **Baxter Earpiece:** Show dialogue directly, vary remote reminders
- **Beats Source:** elements/outlines/episode-02.md Scene 5
- **Acceptance:**
  - File exists with ~1,200-1,400 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Cheryl competitiveness has edge (scene 1/5 of throughline)
  - Exhaustion shown through physical tells
  - Baxter earpiece dialogue varied
  - Opens door: hunt begins, Cheryl pairing / Closes door: solo approach abandoned

---

#### **T007 — Draft Scene 6: Strip Mall Clinic**
- **POV:** Eddie
- **File:** `content/episodes/episode-02-the-wedding-weekend/06-strip-mall-clinic.md`
- **Action:**
  - Open with Eddie on phone with Baxter, mid-conversation
  - Eddie on bench outside clinic
  - Baxter confident clinic is Annabelle's real employer (tax records)
  - Eddie doesn't understand why different company hired them
  - Martinez appears, clocks Eddie: "Funny thing — if I didn't know better, I'd say you were on a stakeout."
  - Eddie scrambles to cover
  - Martinez reveals: they're there because Annabelle worked here, it's sketchy
  - Martinez heads inside
  - Eddie retreats to car, complains to Baxter
  - Button: Investigation moving in public now
- **Character Voice:** Martinez deadpan, measured, slightly amused
- **Sensory:** Tension, frustration of stakeout work
- **Beats Source:** elements/outlines/episode-02.md Scene 6
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Dialogue-first opening (Eddie on phone)
  - Eddie POV maintained (first team POV scene)
  - Martinez voice correct (deadpan, dry wit)
  - Opens door: clinic identified / Closes door: easy access blocked
  - **Ensemble interweaving:** Cuts from hunt (Scene 5) to investigation

---

#### **T008 — Draft Scene 7: The Flower Shop**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/07-the-flower-shop.md`
- **Action:**
  - Open with entering shop or speaking to clerk
  - Doogan and Cheryl hit flower shop (already have clue from Baxter)
  - Jillian + boyfriend creep in to eavesdrop
  - Cheryl notices them
  - **Clue Mechanics (SPECIFIC):**
    1. Cheryl orders red roses → clerk just brings roses (not it)
    2. Baxter reads sonnet: "I have seen roses damasked, red and white"
    3. Doogan orders mix of red and white roses
    4. Clerk asks: "Any color preference?"
    5. Doogan: "Red and white."
    6. Clerk smiles, reaches under counter, hands envelope
  - Next clue: Pride and Prejudice-coded
  - Baxter finds bookstore nearby
  - Turn: Cheryl decides they're trying to crush, not just win
  - Button: Head for bookstore, Jillian still tailing
- **Cheryl Throughline (Scene 2 of 5):**
  - Sharp comment about "younger sisters" or weddings WITH EDGE (not joke—shows pain)
  - Doogan notices but doesn't press
- **Sensory:** Sweat, physical comedy, competition
- **Beats Source:** elements/outlines/episode-02.md Scene 7
- **Acceptance:**
  - File exists with ~1,200-1,400 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Clue mechanics INTERACTIVE (clerk asks, Doogan answers, gets envelope)
  - Cheryl "younger sister" comment present (scene 2/5 throughline)
  - Opens door: first clue solved, next location / Closes door: Jillian threat escalates

---

#### **T008 — Draft Scene 8: Eddie's House**
- **POV:** Eddie or Baxter
- **File:** `content/episodes/episode-02-the-wedding-weekend/08-eddies-house.md`
- **Action:**
  - Open with Eddie on FaceTime with Baxter, mid-conversation
  - Eddie at home, keyed up
  - Baxter explains (too earnestly) he can't "hack a clinic" in vacuum—only bad movies
  - Baxter's point: clinics don't post IT inventory on sign
  - Eddie pushes for magic; Baxter pushes back: needs software names, vendor clues
  - Abby walks in mid-complaint, hears "inventory" and "clinic"
  - Abby immediately understands problem
  - Abby suggests: Baxter impersonate IT tech, go look
  - Abby takes jab at Baxter's vibe (stereotypical nerd, allergic to eye contact)
  - Baxter bristles, then admits she's right
  - Abby agrees to help fake IT uniform (shirt/badge/lanyard)
  - Turn: Abby supplies plan—stop trying to hack mystery box, get software name
  - Button: Baxter in "IT uniform" = idea everyone regrets later
- **Character Voice:** Abby elevated, pragmatic; Eddie pushing; Baxter technical honesty
- **Beats Source:** elements/outlines/episode-02.md Scene 8
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Dialogue-first opening (Eddie on FaceTime)
  - Eddie or Baxter POV maintained
  - Abby voice correct (elevated, sharp)
  - Opens door: clinic infiltration plan / Closes door: no more magic hacking
  - **Timeline alternates:** From hunt (Scene 7) to investigation

---

#### **T009 — Draft Scene 9: Bookstore to Speakeasy**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/09-bookstore-to-speakeasy.md`
- **Action:**
  - Open with entering bookstore or mid-conversation
  - **Summary/Montage Structure:** Compress three stops into one scene
  - Doogan and Cheryl hit bookstore with P&P clue
  - Work through clue, get directed to travel agency
  - **As they arrive at Prohibition Tacos, Doogan summarizes:**
    - Started at bookstore with P&P clue
    - Led to travel agency with Jules Verne quote
    - Travel agency gave them TKAM line
  - Baxter says TKAM points to Prohibition Tacos
  - Doogan: "That doesn't make sense."
  - Baxter insists they'll see
  - Cheryl isn't sure either—just a taco place?
  - Jillian still shadowing
  - Button: Arrive at very small taco place with couple tables—this can't be right
- **Cheryl Throughline (Scene 3 of 5):**
  - Another barbed comment showing wounded pride
  - Physical exhaustion showing through comedy
- **Sensory:** Sweat, exhaustion, physical comedy
- **Beats Source:** elements/outlines/episode-02.md Scene 9
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Summary structure compresses three stops (bookstore/travel agency/taco arrival)
  - Cheryl barbed comment present (scene 3/5 throughline)
  - Opens door: three locations compressed, arrive confused / Closes door: clues resolved
  - **Timeline alternates:** Back to hunt from investigation (Scene 8)

---

#### **T010 — Draft Scene 10: Clinic Montage**
- **POV:** Eddie or Baxter
- **File:** `content/episodes/episode-02-the-wedding-weekend/10-clinic-montage.md`
- **Action:**
  - Open with Baxter in "IT uniform" approaching clinic
  - **Escalating Failures:**
    1. Baxter attempt: temp waves him back, staff clock him, escorted out
    2. Eddie clean route: new patient, charm—temp shuts down
    3. Eddie sneaky route: distraction + photo of monitor—useless (cat videos)
    4. Baxter takes personal offense at cat videos (moral betrayal)
  - Button: Eddie: "Okay. Abby goes in."
- **Sensory:** Tension, frustration, boredom of stakeout/infiltration, physical comedy
- **Beats Source:** elements/outlines/episode-02.md Scene 10
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Action or dialogue opening
  - Eddie or Baxter POV maintained
  - Montage structure compresses failures for pacing
  - Opens door: escalating attempts / Closes door: Eddie/Baxter methods exhausted
  - **Timeline alternates:** From hunt (Scene 9) to investigation

---

#### **T011 — Draft Scene 11: Prohibition Speakeasy**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/11-prohibition-speakeasy.md`
- **Action:**
  - Open with Baxter speaking or Doogan at taco counter
  - Prohibition Tacos looks like small taco place
  - Baxter: found it—Reddit post says ask for mockingbird tacos
  - Doogan asks; clerk gestures toward hidden door
  - Enter Prohibition Speakeasy (prohibition-era bar hidden behind taco place)
  - Jillian + boyfriend already there with champagne—how are they that fast?
  - But they look lost, confused
  - **Clue Mechanics (SPECIFIC):**
    1. Bartender offers champagne
    2. Cheryl about to accept (Jillian has champagne, must be right)
    3. Doogan notices trick—declines: "We're not really drinkers."
    4. Bartender smiles, slides water bottle
    5. Coaster underneath is golden ticket
  - As they leave, Cheryl: "So you don't drink?"
  - Doogan: "Hell no. I drink a lot. Atticus Finch didn't drink in the book."
  - Cheryl impressed—starts warming to him
  - Turn: Doogan's literary knowledge wins round and earns respect
  - Button: They have golden ticket; Jillian stuck with champagne
- **Cheryl Throughline (Scene 4 of 5):**
  - Starts to be genuinely impressed by Doogan (TURNING POINT)
  - Competitive fire still present but respect building
- **Sensory:** Sweat, competition, surprise/discovery of hidden bar
- **Beats Source:** elements/outlines/episode-02.md Scene 11
- **Acceptance:**
  - File exists with ~1,400-1,600 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Clue mechanics INTERACTIVE (champagne offer → decline → water with ticket)
  - Cheryl warming to Doogan (scene 4/5 throughline, TURNING POINT)
  - Opens door: hidden speakeasy discovered / Closes door: Cheryl warming (relationship arc turns)
  - **Timeline alternates:** Back to hunt from investigation (Scene 10)

---

#### **T012 — Draft Scene 12: Clinic Success**
- **POV:** Abby
- **File:** `content/episodes/episode-02-the-wedding-weekend/12-clinic-success.md`
- **Action:**
  - Open with Abby entering or speaking to temp, already engaging
  - Abby goes in alone, no theatrics
  - Makes temp laugh inside 30 seconds (effortless small talk)
  - Simple pretext (billing question, prescription, "I was told to ask front desk")
  - Lets temp overshare and swivel monitor
  - Abby clocks clinic software name (exactly what Baxter needs)
  - Abby asks when regular receptionist is back
  - Temp gives vague answer (kept in dark)
  - Abby leaves like she ran errand
  - Immediately texts/calls software name to team
  - Button: Mission accomplished through competence, not heroics
- **Character Voice:** Abby doesn't sound like spy—sounds like competent person
- **Sensory:** Calm, effortless competence (no heroics)
- **Beats Source:** elements/outlines/episode-02.md Scene 12
- **Acceptance:**
  - File exists with ~800-1,000 words
  - Dialogue-first opening (Abby engaging)
  - Abby POV maintained (first solo Abby POV scene)
  - Abby voice correct (elevated, pragmatic, effortless)
  - Opens door: software name acquired / Closes door: Eddie/Baxter failures resolved
  - **Timeline alternates:** From hunt (Scene 11) to investigation

---

#### **T013 — Draft Scene 13: Finish Line**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/13-finish-line.md`
- **Action:**
  - Open with arriving or Terry speaking
  - Doogan and Cheryl arrive back at country club
  - Jillian + boyfriend already there—smug, stumped
  - Golden ticket has "Quid agis?" on back (Latin)
  - Terry asks (casual): "Are you done?"
  - Cheryl about to answer with something profane and victorious
  - Doogan stops her: "It's a trick."
  - Doogan needles Sylvia: not her degree—her childhood
  - Sylvia mentions mom reading Harry Potter
  - Baxter (in ear) connects: password/painting/phrase
  - Terry asks again
  - Doogan answers: "Not until the fat lady sings."
  - Turn: They win
  - Button: Jillian furious. Doogan carries flashy golf driver. Cheryl clutching handbag, says she could use ride to rehearsal dinner
- **Cheryl Throughline (Scene 5 of 5):**
  - Victory moment shows competitive drive masking something deeper
  - Invitation to rehearsal dinner = trust building, emotional guard still up
- **Sensory:** Exhaustion, competition triumph
- **Beats Source:** elements/outlines/episode-02.md Scene 13
- **Acceptance:**
  - File exists with ~1,200-1,400 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Clue mechanics INTERACTIVE (Terry asks "Are you done?" → answer "Not until the fat lady sings")
  - Cheryl throughline complete (scene 5/5, invitation to dinner)
  - Opens door: hunt victory, dinner invitation / Closes door: hunt sequence complete
  - **Timeline alternates:** Back to hunt from investigation (Scene 12)

---

#### **T014 — Draft Scene 14: Eddie and Baxter**
- **POV:** Eddie or Baxter
- **File:** `content/episodes/episode-02-the-wedding-weekend/14-cross-a-few-lines.md`
- **Action:**
  - Open with Eddie energized about software name, mid-conversation
  - Coming off Abby's win, Eddie energized
  - Baxter less excited—because now it's possible (and illegal)
  - Eddie pushes: with Doogan under suspicion, can't wait for subpoenas
  - Baxter resists on principle and fear; Doogan hates when Baxter crosses lines
  - Eddie frames bluntly: unless they want Doogan in prison for life, "clean" isn't option
  - Baxter agrees reluctantly, sets boundary
  - Eddie immediately tries to negotiate boundary
  - Button: Pivot scene—moral line about to be crossed
- **Character Voice:** Eddie motor-mouth steamrolling Baxter's objections, tangents circle back to "Doogan can't go to prison"
- **Beats Source:** elements/outlines/episode-02.md Scene 14
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Dialogue-first opening (Eddie energized)
  - Eddie or Baxter POV maintained
  - Eddie motor-mouth voice present
  - Opens door: moral stakes (illegal system access) / Closes door: Baxter agrees
  - **Character moment:** Eddie's loyalty overrides Baxter's caution

---

#### **T016 — Draft Scene 15: Rehearsal Dinner**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/15-rehearsal-dinner.md`
- **Action:**
  - Open with dinner conversation already in progress
  - Doogan seated with Cheryl; she's warmed up now
  - **Reveal (MUST FEEL INEVITABLE, NOT SURPRISING):**
    - Cheryl didn't get dumped—she dumped Brad
    - Gabby (younger sister) swooped in, now marrying him
  - Doogan realizes real job: containment, not distraction
  - Keeps Cheryl buoyant through dinner without breaking Secrecy Rule
  - Takes Cheryl home
  - She's about to invite him in
  - Doogan spots Martinez in car doing surveillance
  - Turn: Martinez surveillance costs Doogan the Cheryl connection
  - Button: Doogan waves to Martinez on way back to car. Martinez smiles, waves back.
- **Setting:** Trendy coastal restaurant, Malibu old money vibe
- **Cheryl Reveal:** Must feel inevitable from earlier "younger sister" comments
- **Beats Source:** elements/outlines/episode-02.md Scene 15
- **Acceptance:**
  - File exists with ~1,400-1,600 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Cheryl reveal feels inevitable (payoff from scenes 5/7/8/9/10)
  - Martinez surveillance costs connection (not forced)
  - Opens door: true wedding stakes revealed / Closes door: romantic possibility ended
  - **Discovery:** Job was containment, not distraction

---

#### **T017 — Draft Scene 16: Martinez Ambush**
- **POV:** Martinez
- **File:** `content/episodes/episode-02-the-wedding-weekend/16-martinez-ambush.md`
- **Action:**
  - Open with Martinez approaching or speaking
  - Arriving at home, Martinez parks and approaches Doogan
  - "Are your friends investigating the clinic?"
  - Martinez has pictures of Eddie, Baxter, Abby at clinic
  - "Are you following us?" / "Yeah, that's how investigations work."
  - Martinez suggests letting real cops do the work
  - Doogan asks where Finch is
  - Martinez dismisses: "what is it with you and buddy cop movies"
  - Martinez admits things don't add up but Doogan still prime suspect
  - Button: Martinez: "I am going to have more questions. Don't leave town."
- **Character Voice:** Martinez deadpan, measured, dry wit, calm authority
- **Beats Source:** elements/outlines/episode-02.md Scene 16
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Dialogue-first opening
  - Martinez POV maintained (first Martinez POV scene)
  - Martinez voice correct (deadpan, principled)
  - Opens door: Martinez aware of team investigation / Closes door: Doogan/Cheryl romance fully ended
  - **Discovery:** Martinez suspects something else but can't prove innocence

---

#### **T018 — Draft Scene 17: Reception**
- **POV:** Doogan
- **File:** `content/episodes/episode-02-the-wedding-weekend/17-reception.md`
- **Action:**
  - Open with reception already underway
  - Ceremony went fine; now reception
  - Terry + assistant scramble with last-minute problems
  - Cheryl drinking hard
  - Doogan keeps "losing" her drink whenever he can
  - Cheryl threatens catastrophic toast
  - Doogan talks her off ledge
  - Cheryl gives sweetest toast she's capable of—it lands
  - Button: Job complete. Cheryl contained, wedding saved.
- **Tone:** Funny and fast, not sentimental
- **Beats Source:** elements/outlines/episode-02.md Scene 17
- **Acceptance:**
  - File exists with ~1,200-1,400 words
  - Dialogue-first opening
  - Doogan POV maintained
  - Tone: funny/fast, not sentimental
  - Opens door: reception chaos, toast threat / Closes door: job complete
  - **Payoff:** Doogan succeeds at containment despite losing connection

---

#### **T019 — Draft Scene 18: The File That Vanished**
- **POV:** Baxter (with Eddie/Abby present)
- **File:** `content/episodes/episode-02-the-wedding-weekend/18-the-file-that-vanished.md`
- **Action:**
  - Open with Baxter announcing success or frustration
  - Eddie and Abby in living room
  - Baxter with them on laptop
  - Baxter finally gets into clinic medical software
  - Frustrated, then triumphant
  - Finds file Annabelle flagged on her last day
  - Annabelle's note: "Something isn't right about this. Need to double check."
  - As Baxter starts reading, file disappears in real time
  - Baxter checks active sessions: someone else on system right now deleting files
  - Button/Cliffhanger: Someone is actively covering tracks RIGHT NOW
- **NOTE:** Doogan is NOT present (team POV only)
- **Beats Source:** elements/outlines/episode-02.md Scene 18
- **Acceptance:**
  - File exists with ~1,000-1,200 words
  - Dialogue-first opening
  - Baxter POV maintained (Eddie/Abby present but Baxter POV)
  - Doogan NOT present
  - Opens door: file discovered with Annabelle's note / Closes door: file deleted (villain active NOW)
  - **Cliffhanger:** Episode ends on active threat
  - **Stephen King:** Discovery happens in real time, villain presence revealed through action

---

### Phase 3: QA Pass

**T020 — QA Pass: Pre-Writing Verification Checklist**
- **Action:** Run full checklist from elements/checklist.md against all 18 scenes
- **Checks:**
  1. POV purity (no head-hopping, no first-person, no villain POV)
  2. McDonald-style openings (no atmospheric intros)
  3. Character voice consistency (Eddie motor-mouth, Doogan measured, Abby elevated, Martinez deadpan)
  4. Cheryl throughline present in Scenes 5, 7, 8, 9, 10 (seeds → reveal Scene 15)
  5. Clue mechanics interactive (Scenes 7, 9, 10)
  6. Fair-play information flow
  7. Sensory details present
  8. Baxter earpiece dialogue varied
  9. Continuity with Episode 1
  10. Serial beats (hook/escalation/cliffhanger)
- **Acceptance:** All 18 scenes pass checklist

---

**T021 — QA Pass: Stephen King Principles**
- **Action:** Apply Stephen King craft review checklist to all 18 scenes
- **Checks:**
  1. Does each scene open/close doors?
  2. Is character behavior honest (not plot-convenient)?
  3. Is there sensory detail?
  4. Is there emotional truth?
  5. Does it open with dialogue or action (no atmospheric setup)?
  6. Can 10-20% be cut at paragraph level?
  7. Are adverbs necessary?
  8. Does discovery feel earned?
- **Acceptance:** All scenes pass Stephen King principles review

---

**T022 — QA Pass: Ensemble Structure Verification**
- **Action:** Verify ensemble storytelling structure maintains optimal rhythm
- **Checks:**
  1. Scene 6 provides early hunt/investigation interweaving
  2. Hunt scenes 7-10 build uninterrupted (Cheryl arc)
  3. Investigation scenes 11-14 escalate systematically
  4. Neither track disappears for more than 4 scenes
  5. Convergence scenes 15-18 pay off both tracks
  6. POV distribution: Doogan 11 scenes, Team 7 scenes
  7. No confusion about which track reader is following
- **Acceptance:** Episode maintains optimal ensemble rhythm

---

### Phase 4: Finalization

**T023 — Compile Episode 2 for Output**
- **Action:** Compile all 18 scene files into single output file
- **File:** `output/episode-02-the-wedding-weekend.md`
- **Process:**
  1. Concatenate all scene files in order 1-18
  2. Add episode header (title, episode number, word count)
  3. Verify scene breaks are clear
- **Acceptance:** Single compiled episode file exists in output/

---

## Summary

**Total Tasks:** 23  
**Scene Files:** 18 (01-18)  
**Estimated Total Word Count:** ~20,000-22,000 words (~1,100 words/scene average)

**Critical Path:**
1. Create directory (T001)
2. Draft scenes sequentially 1-18 (T002-T019)
3. QA pass all scenes (T020-T022)
4. Compile output (T023)

**Quality Gates:**
- Every scene must pass Pre-Writing Verification Checklist (elements/checklist.md)
- Every scene must pass Stephen King principles review (stephen-king-review.md)
- Ensemble structure must maintain optimal rhythm (plan.md analysis)

**Ready for implementation.** All prerequisites satisfied. No blocking issues.
